...
export default function ReleaseDescription({ description }: { description: string }) {
  const [expandedByDescription, setExpandedByDescription] = useState<{
    description: string;
    expanded: boolean;
  }>({
    description: "",
  ...
  const expanded =
    expandedByDescription.description === normalized ? expandedByDescription.expanded : false;
  ...
  return (
    <div className="mt-4">
      <div
        className={canCollapse && !expanded ? "max-h-36 overflow-hidden" : undefined}
        style={canCollapse && !expanded ? COLLAPSED_FADE_MASK : undefined}
      >
        <div ref={contentRef} className="space-y-3">
          {blocks.map((block, index) => (
            <Fragment key={`release-description-${index}`}>{renderBlock(block, `block-${index}`)}</Fragment>
          ))}
        </div>
      </div>
      {canCollapse ? (
