...
describe("POST /api/checkout/session", () => {
  ...
  it("enforces a paid PWYW minimum that covers Stripe fees plus one cent", async () => {
    process.env.STRIPE_SECRET_KEY = "sk_test_dummy";
    process.env.DATABASE_URL ??=
      "postgresql://postgres:postgres@localhost:5432/merch_table_test";
    ...
    try {
      process.env.MINIMUM_PRICE_FLOOR_CENTS = "1";
      const { prisma } = await import("@/lib/prisma");
      ...
      assert.deepEqual(await response.json(), {
        ok: false,
        error:
          "For PWYW, enter 0 cents for a free checkout or at least 32 cents for Stripe checkout.",
      });
    } finally {
